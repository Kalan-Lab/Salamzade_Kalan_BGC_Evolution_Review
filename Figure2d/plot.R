...
pdf('Figure_2D.pdf', height=3, width=2)
ggplot(dat, aes(x=bgc_related, y=tajimas_d, fill=bgc_related)) + geom_violin(draw_quantiles = c(0.5), show.legend=F) + theme_classic() + scale_fill_manual(values=c('#e0dede', '#8c8c8c')) + xlab("") + ylab("") 
